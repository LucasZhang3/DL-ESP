...
auto CRenderStackSystem::EnsureUpdateCapacity( const size_t n ) -> void
{
	std::scoped_lock lock( m_Lock );

	if ( n > m_vecUpdateBuffer.capacity() )
		m_vecUpdateBuffer.reserve( n );
}
...
auto CRenderStackSystem::OnClientOutput() -> void
{
	...
	{
		static const auto s_pEmptyCommands = std::make_shared<RenderObjectsVec_t>();

		m_pSharedBuffer.store( s_pEmptyCommands );
		m_bSharedBufferReady = true;
		return;
	}

	m_pSharedBuffer.store( std::make_shared<RenderObjectsVec_t>( std::move( m_vecUpdateBuffer ) ) );
	m_bSharedBufferReady = true;
}
