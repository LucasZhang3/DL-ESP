...
static CDeadlockGUI g_CDeadlockGUI{};

IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler( HWND hwnd , UINT msg , WPARAM wParam , LPARAM lParam );

auto CDeadlockGUI::OnInit( IDXGISwapChain* pSwapChain ) -> void
{
	DXGI_SWAP_CHAIN_DESC SwapChainDesc;

	if ( FAILED( pSwapChain->GetDevice( IID_PPV_ARGS( &m_pDevice ) ) ) )
	{
		DEV_LOG( "[error] CDeadlockGUI::OnInit: #1\n" );
		return;
	}
	...
	{
		DEV_LOG( "[error] CDeadlockGUI::OnInit: #2\n" );
		return;
	}
...
auto CDeadlockGUI::OnDestroy() -> void
{
	SetWindowLongPtrA( m_hDeadLockWindow , GWLP_WNDPROC , (LONG_PTR)GetDeadlockGUI()->m_WndProc_o );
...
auto CDeadlockGUI::InitFont() -> void
{
...
auto CDeadlockGUI::InitIndigoStyle() -> void
{
...
auto CDeadlockGUI::InitVermillionStyle() -> void
{
	auto& style = ImGui::GetStyle();
	auto& colors = style.Colors;
...
auto CDeadlockGUI::InitClassicSteamStyle() -> void
{
...
auto CDeadlockGUI::UpdateStyle() -> void
{
	ImGui::SetCurrentContext( m_pImGuiContext );

	if ( Settings::Misc::MenuStyle == EDeadlockGuiStyle::INDIGO )
		InitIndigoStyle();
	else if ( Settings::Misc::MenuStyle == EDeadlockGuiStyle::VERMILLION )
		InitVermillionStyle();
	else if ( Settings::Misc::MenuStyle == EDeadlockGuiStyle::CLASSIC_STEAM )
		InitClassicSteamStyle();
	else if ( Settings::Misc::MenuStyle == EDeadlockGuiStyle::CHARCOAL )
		InitCharcoalStyle();
}

auto CDeadlockGUI::InitCharcoalStyle() -> void
{
	auto& style = ImGui::GetStyle();
	auto& colors = style.Colors;
	...
	const ImVec4 text = ImVec4( 0.92f , 0.92f , 0.93f , 1.00f );
	const ImVec4 textDisabled = ImVec4( 0.45f , 0.46f , 0.48f , 1.00f );
	const ImVec4 windowBg = ImVec4( 0.07f , 0.07f , 0.08f , 1.00f );
	const ImVec4 childBg = ImVec4( 0.09f , 0.09f , 0.10f , 1.00f );
	const ImVec4 popupBg = ImVec4( 0.08f , 0.08f , 0.09f , 1.00f );
	const ImVec4 border = ImVec4( 0.20f , 0.21f , 0.23f , 1.00f );
	const ImVec4 frameBg = ImVec4( 0.11f , 0.11f , 0.12f , 1.00f );
	const ImVec4 frameBgHover = ImVec4( 0.14f , 0.14f , 0.16f , 1.00f );
	const ImVec4 frameBgActive = ImVec4( 0.16f , 0.16f , 0.18f , 1.00f );
	const ImVec4 accent = ImVec4( 0.28f , 0.58f , 0.62f , 1.00f );
	const ImVec4 accentHover = ImVec4( 0.34f , 0.66f , 0.70f , 1.00f );
	const ImVec4 accentActive = ImVec4( 0.22f , 0.50f , 0.54f , 1.00f );
	...
	colors[ImGuiCol_TitleBgCollapsed] = ImVec4( 0.06f , 0.06f , 0.07f , 0.82f );

	colors[ImGuiCol_MenuBarBg] = childBg;

	colors[ImGuiCol_ScrollbarBg] = ImVec4( 0.06f , 0.06f , 0.07f , 1.00f );
	colors[ImGuiCol_ScrollbarGrab] = ImVec4( 0.22f , 0.23f , 0.25f , 1.00f );
	...
	colors[ImGuiCol_ButtonHovered] = frameBgHover;
	...
	colors[ImGuiCol_PlotLines] = ImVec4( 0.55f , 0.55f , 0.56f , 1.00f );
	colors[ImGuiCol_PlotLinesHovered] = accentHover;
	colors[ImGuiCol_PlotHistogram] = accent;
	colors[ImGuiCol_PlotHistogramHovered] = accentHover;

	colors[ImGuiCol_TextSelectedBg] = ImVec4( accent.x , accent.y , accent.z , 0.35f );
	colors[ImGuiCol_DragDropTarget] = ImVec4( 0.95f , 0.95f , 0.30f , 0.90f );

	colors[ImGuiCol_NavHighlight] = accent;
	colors[ImGuiCol_NavWindowingHighlight] = ImVec4( 1.00f , 1.00f , 1.00f , 0.70f );
	colors[ImGuiCol_NavWindowingDimBg] = ImVec4( 0.20f , 0.20f , 0.22f , 0.55f );

	colors[ImGuiCol_ModalWindowDimBg] = ImVec4( 0.10f , 0.10f , 0.11f , 0.60f );
	colors[ImGuiCol_WindowShadow] = ImVec4( 0.f , 0.f , 0.f , 0.55f );
}

void CDeadlockGUI::OnPresent( IDXGISwapChain* pSwapChain )
{
	if ( !m_bInit )
		OnInit( pSwapChain );
	else
...
void CDeadlockGUI::OnResizeBuffers( IDXGISwapChain* pSwapChain )
{
	OnDestroy();
}

void CDeadlockGUI::OnRender( IDXGISwapChain* pSwapChain )
{
	...
	else
	{
		if ( !m_pRenderTargetView )
		{
			...
			{
				DEV_LOG( "[error] CDeadlockGUI::OnRender: #1\n" );
				return;
			}
...
auto CDeadlockGUI::OnReopenGUI() -> void
{
...
LRESULT WINAPI CDeadlockGUI::GUI_WndProc( HWND hwnd , UINT uMsg , WPARAM wParam , LPARAM lParam )
{
	...
	if ( GetDeadlockGUI()->m_bInit )
	{
		if ( uMsg == WM_KEYUP && wParam == VK_INSERT )
			GetDeadlockGUI()->OnReopenGUI();

		if ( GetDeadlockGUI()->IsVisible() && ImGui_ImplWin32_WndProcHandler( hwnd , uMsg , wParam , lParam ) == 0 )
			return true;
	}

	return CallWindowProcA( GetDeadlockGUI()->m_WndProc_o , hwnd , uMsg , wParam , lParam );
}

auto CDeadlockGUI::FreeTypeBuild::PreNewFrame() -> bool
{
	if ( !WantRebuild )
		return false;
...
auto CDeadlockGUI::ClearRenderTargetView() -> void
{
	if ( m_pRenderTargetView )
	{
...
auto GetDeadlockGUI() -> CDeadlockGUI*
{
	return &g_CDeadlockGUI;
}
