...
auto CFont::InitFont( IFW1Factory* pFW1Factory , std::wstring Name , float FontSize ) -> void
{
	...
	if ( FAILED( pFW1Factory->CreateFontWrapper( GetDeadlockGUI()->GetDevice() , NULL , &createParams , &m_pFontWrapper ) ) )
	{
...
auto CFont::DrawString( int x , int y , ImColor Color , int Flags , const char* fmt , ... ) -> void
{
	...
	if ( m_pFontWrapper )
	{
		...
		ID3D11DeviceContext* pContext = GetDeadlockGUI()->GetDeviceContext();
		...
		GetDeadlockGUI()->GetDevice()->CreateBlendState( &blendDesc , &pBlendState );
		...
		{
			m_pFontWrapper->DrawString( GetDeadlockGUI()->GetDeviceContext() ,
